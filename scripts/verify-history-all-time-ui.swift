...
private func launchFreshApp() throws -> NSRunningApplication {
    ...
    guard semaphore.wait(timeout: .now() + 10) == .success else {
        throw NSError(domain: "HistoryAllTimeUISmoke", code: 1, userInfo: [
            NSLocalizedDescriptionKey: "Timed out launching Dictator-md"
        ])
    }
    if let launchError { throw launchError }
    guard let launchedApp else {
        throw NSError(domain: "HistoryAllTimeUISmoke", code: 2, userInfo: [
...
private func verifyHistoryRanges(in window: AXUIElement, width: Int, height: Int) throws {
    ...
    guard pressButton(label: "Open History", in: window) else {
        throw NSError(domain: "HistoryAllTimeUISmoke", code: 3, userInfo: [
            NSLocalizedDescriptionKey: "History navigation button is missing or not pressable"
        ])
    }
    Thread.sleep(forTimeInterval: 0.5)
    ...
    let monthLabels = visibleLabels.filter { $0.hasPrefix("All-time month:") }

    guard allTimeCardLabels.count >= 6 else {
        throw NSError(domain: "HistoryAllTimeUISmoke", code: 5, userInfo: [
            NSLocalizedDescriptionKey: "Expected six all-time metric cards at \(width)x\(height), found \(allTimeCardLabels.count)"
        ])
    }
    guard !monthLabels.isEmpty else {
        throw NSError(domain: "HistoryAllTimeUISmoke", code: 6, userInfo: [
            NSLocalizedDescriptionKey: "All-time monthly timeline did not render at \(width)x\(height)"
        ])
    }
...
do {
    ...
    guard waitUntil(timeout: 5, condition: { mainWindow(of: appElement) != nil }),
        ...
        throw NSError(domain: "HistoryAllTimeUISmoke", code: 7, userInfo: [
