...
int App::run(HINSTANCE instance, int showCommand) {
    ...
    if (!tray_.create(window_, instance)) {
        MessageBoxW(nullptr, L"Failed to create Dictator-md tray icon.", L"Dictator-md", MB_ICONERROR);
        return 1;
    }

    if (!hotkey_.registerDefault(window_)) {
        MessageBoxW(nullptr, L"Failed to register the Right Alt hotkey.", L"Dictator-md", MB_ICONWARNING);
    }
